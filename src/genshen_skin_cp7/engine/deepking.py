...
def pkg_root():
    """包目录 .../genshen_skin_cp7(本模块在它的 engine/ 下, 故向上一层)。
...
# src/client/genshen-cp7.module.css 完全一致的取值, 保证在线转换与离线生成一致。
...
FALLBACK_CSS = """:root {
  ...
  --bg-layer-2: #eef3fa;
  --bg-layer-3: #dde7f4;
  --label-primary: #1e2436;
  --label-secondary: #4d5670;
  --label-tertiary: #7d879e;
  --brand-primary: #b8863f;
  --accent: #b8863f;
  --primary: #b8863f;
  --interactive-bg-hover-solid: #f2e8da;
  --interactive-bg-active: #e5d5bd;
  --border-l1: #eadfce;
  --border-l2: #dccdb4;
  --border: #dccdb4;
}
        ...
        <section class="mock" style="background:%(bg)s;color:%(bgText)s">
          ...
          <div class="body">
            <aside style="background:%(sidebarBg)s;color:%(sidebarText)s;border-color:%(border)s">
              ...
              <div class="item">📁 Genshen-skin-CP7</div>
...
<h2>🌙 暗色 Dark</h2>%(dark)s
        ...
        "light": region(p, "亮色 · 原神CP7", mascot.get("light")),
        "dark": region(d, "暗色 · 原神CP7", mascot.get("dark")),
    }
