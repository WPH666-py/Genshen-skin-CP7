...
本目录的 `genshen-cp7.module.css` 同时服务两个消费者，改它之前请读完本文。
...
`genshen-cp7.module.css` 的注释里绝对不能出现：
...
- 想保留夜景蓝紫的逐槽位校色版本，用：
  ...
  ```bash
  genshen-cp7 deepking          # 导出 ~/.genshen-cp7/deepking/*.skin.json
  ```
...
```bash
genshen-cp7 deepking --check    # 应输出「通过」
genshen-cp7 deepking --what     # 看转换器实际会取到什么值
```
...
- `mascot-cp7-light.jpg` —— 取「花园」那张的淡彩立绘, 配浅色界面
- `mascot-cp7-dark.jpg` —— 取「下午茶」那张的对坐构图, 配深色界面
