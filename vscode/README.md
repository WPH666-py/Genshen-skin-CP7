# 原神CP7 — 娜维娅×克洛琳德 · VSCode / Trae / CodeX 扩展
...
```bash
code --install-extension genshen-skin-cp7-0.1.0.vsix
```
...
1. 活动栏点 **原神CP7** 图标 → 打开「皮肤画廊」
2. 点任意卡片的「设为壁纸」即可切换到该系统壁纸
3. 底部四个按钮: 随机换一张 / 生成全部样式 / 壁纸切换器 / 桌面桌宠

命令面板(`Ctrl+Shift+P`)搜 `原神CP7` 也能用全部命令。

## 依赖

- Python 3.9+ 与 Pillow(扩展检测到 Pillow 缺失会**自动** `pip install pillow`)
- 本套件的 Python 包: `pip install genshen-skin-cp7`

扩展按以下顺序查找 Python 包: 已 pip 安装 → `genshen_skin_cp7.repoPath` 设置项 →
`%USERPROFILE%\Genshen-skin-CP7` → 当前工作区。
...
| `genshincp1.repoPath` | 空 | 仓库路径(含 `src/genshen_skin_cp7`), 留空自动检测 |
